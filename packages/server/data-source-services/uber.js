...
const apiUber = require('../data-source-apis/uber');
...
function authenticate(code) {
  return apiUber.oauth.token({ code })
    .then(data => {
...
function refreshAuth(auth) {
  return apiUber.oauth.refresh(auth)
    .then(data => {
      const expiry_time = calculateExpiryTime(data.expires_in);
      return { expiry_time, ...data };
    });
...
function getHistory (from, to, token) {
  ...
  return history(offset, limit, token, fromTime, initialResults)
    .then(results => results.filter(result => (result.start_time >= fromTime) && (result.end_time <= toTime)));
}

function history (offset, limit, token, fromTime, results) {
  return apiUber.history({ offset, limit, access_token: token })
    .then(response => {
      // Get the minTime from response.history
      const minTime = response.history.reduce((min, ride) => ride.start_time < min ? ride.start_time : min, response.history[0].start_time);
      ...
      if (fromTime > minTime || exhausted) {
        ...
        // Keep checking
        return history(offset + limit, limit, token, fromTime, results.concat(response.history))
      }
    })
}
